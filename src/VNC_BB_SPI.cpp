...
void BB_SPI_VNC::area_update_data(char * data, uint32_t pixel) {
    ...
    if(pixel > max_spi) {
        uint32_t remain = pixel;
        while(remain > 0) {
            uint32_t chunk = min(remain, max_spi);
            BB_SPI_LCD::pushPixels((uint16_t *)data+(pixel-remain), chunk, DRAW_TO_LCD);
            remain -= chunk;
        }
        return;
    } else {
        BB_SPI_LCD::pushPixels((uint16_t *)data, pixel, DRAW_TO_LCD);
    }
}
